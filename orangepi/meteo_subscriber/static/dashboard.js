...
function updateDht22ToggleButton() {
    ...
    if (dht22Visible) {
        btn.innerHTML = 'Hide Extra';
    } else {
        btn.innerHTML = 'Show Extra';
    }
}
                    ...
                    <tbody id="latest-data-tbody">
                        ${data.data.map(row => {
                            ...
                            <tr data-timestamp="${row.timestamp_server}">
                                ...
                                <td>${formatValue(row.aht20_temperature_c, 1)}</td>
                                <td>${formatValue(row.aht20_humidity_percent, 1)}</td>
...
async function loadHistoricalData() {
    try {
        ...
        // Use aggregated data for longer time ranges to avoid hitting limits
        if (selectedTimeRange >= 72) {
            // For 3+ days, use aggregation
            const intervalMinutes = selectedTimeRange >= 168 ? 10 : 3; // 10min for week, 3min for 3 days
            ...
            console.log(`Loading aggregated data for ${selectedTimeRange} hours with ${intervalMinutes}min intervals...`);
            response = await fetch(`/api/data/aggregated?${params}`);
            data = await response.json();
        } else {
            // For shorter periods, use raw data
            const estimatedLimit = Math.max(1000, selectedTimeRange * 250);
            ...
            console.log(`Loading historical data for ${selectedTimeRange} hours with limit ${estimatedLimit}...`);
            response = await fetch(`/api/data/history?${params}`);
            data = await response.json();
        }
        
        console.log(`Received ${data.data.length} data points for ${data.hours} hours`);
        if (data.data.length > 0) {
            const oldest = new Date(data.data[data.data.length - 1].timestamp_server * 1000);
            const newest = new Date(data.data[0].timestamp_server * 1000);
        ...
        renderChart('temp-chart-aht20', 'AHT20 Temperature (°C)', datasets, 'aht20_temperature_c');
        renderChart('humidity-chart-aht20', 'AHT20 Humidity (%)', datasets, 'aht20_humidity_percent');
...
function getFieldForChart(chartId) {
    const fieldMap = {
        'temp-chart-aht20': 'aht20_temperature_c',
        'humidity-chart-aht20': 'aht20_humidity_percent',
...
function renderChart(chartId, title, datasets, field) {
    ...
    const chartDatasets = datasets.map(ds => {
        ...
        console.log(`  ${ds.deviceId}: ${recentPoints.length} after time filtering`);
        
        // Apply both absolute and relative filtering
        const filteredPoints = filterOutliersByMovingAverage(recentPoints, field);
        
        console.log(`  ${ds.deviceId}: ${filteredPoints.length} after outlier filtering`);
...
function renderPressureTrendChart(chartId, datasets) {
    ...
    const cutoffTime = Date.now() / 1000 - (selectedTimeRange * 3600);
    
    // Calculate pressure trends for each device
    const chartDatasets = datasets.map(({deviceId, color, rows}) => {
        // Filter rows by time range first
        const recentRows = rows.filter(row => row.timestamp_server > cutoffTime);
        ...
        for (let i = 0; i < recentRows.length; i++) {
            ...
            for (let j = i - 1; j >= 0; j--) {
                const timeDiff = currentTime - recentRows[j].timestamp_server;
...
function renderHeapChart(chartId, datasets) {
    ...
    const chartDatasets = datasets.map(({deviceId, color, rows}) => {
        const heapData = rows
            ...
            .filter(row => row.timestamp_server * 1000 > cutoffTime)
            .map(row => ({
                x: row.timestamp_server * 1000,
